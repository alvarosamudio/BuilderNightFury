...
#Install Kali Repoditory
apt-key adv --keyserver pool.sks-keyservers.net --recv-keys ED444FF07D8D0BF6
echo '# Kali linux repositories \ndeb http://http.kali.org/kali kali-rolling main contrib non-free' >> /etc/apt/sources.list

apt-get update && apt-get install -y --no-install-recommends linux-image-amd64 live-boot systemd-sysv
...
apt-get install -y --no-install-recommends network-manager net-tools wireless-tools wpagui curl openssh-server openssh-client tor privoxy gparted gpart\
    xserver-xorg-core xserver-xorg xinit xterm screenfetch screen lxterminal vim lightdm awesome awesome-extra nano && apt-get clean
